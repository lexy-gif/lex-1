...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    academic_query($dbh,'UPDATE tblparentnotifications n JOIN tblparentstudents ps ON ps.ParentId=n.ParentId AND ps.StudentId=n.StudentId AND ps.Status=1 JOIN tblstudents s ON s.StudentId=n.StudentId AND s.Status=1 SET n.ReadAt=COALESCE(n.ReadAt,NOW()) WHERE n.ParentId=? AND (?=1 OR n.id=?)',[$parent['id'],$all?1:0,$id]);
