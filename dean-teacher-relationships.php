...
if ($ready && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $action=$_POST['action']??'';
    $required=['subject'=>'teaching.assign','class'=>'assignments.class','responsibility'=>'assignments.responsibility','type'=>'responsibilities.manage','type_status'=>'responsibilities.manage','scope'=>'responsibilities.manage'][$action]??null;
    if($action==='end')$required=['subject'=>'teaching.assign','class'=>'assignments.class','responsibility'=>staff_can('responsibilities.manage')?'responsibilities.manage':'assignments.responsibility'][$_POST['kind']??'']??null;
    if($action==='responsibility'&&staff_can('responsibilities.manage'))$required='responsibilities.manage';
    staff_require($required);
    try {
        ...
        elseif ($action==='type') {
            ...
            if(!academic_query($dbh,'SELECT id FROM tblresponsibilitytypes WHERE id=?',[$type])->fetchColumn())throw new DomainException('Unknown responsibility type.');
            academic_query($dbh,'INSERT INTO tblresponsibilityscope(ResponsibilityTypeId,IsAcademic) VALUES(?,?) ON DUPLICATE KEY UPDATE IsAcademic=VALUES(IsAcademic)',[$type,(int)!empty($_POST['academic'])]);
        ...
        elseif ($action==='end') {
            ...
            if($kind==='responsibility'&&!staff_can('responsibilities.manage')&&!academic_query($dbh,'SELECT r.id FROM tblteacherresponsibilities r JOIN tblresponsibilityscope sc ON sc.ResponsibilityTypeId=r.ResponsibilityTypeId AND sc.IsAcademic=1 WHERE r.id=?',[(int)($_POST['assignment']??0)])->fetchColumn())staff_forbid();
...
<p><?php if(staff_can('accounts.teachers')) { ?><a class="btn btn-default" href="manage-teachers.php#create-teacher">Add Teacher</a><?php } ?> <a class="btn btn-default" href="manage-subjects.php">Manage Subjects</a> <a class="btn btn-default" href="student-subjects.php">Student Subject Combinations</a></p>
...
if($kind==='responsibility'&&!staff_can('responsibilities.manage'))$where.=' AND EXISTS(SELECT 1 FROM tblresponsibilityscope sc WHERE sc.ResponsibilityTypeId=a.ResponsibilityTypeId AND sc.IsAcademic=1)';
...
<button class="btn btn-primary" name="action" value="subject">Assign Subject Teacher</button> <?php if(staff_can('assignments.class')) { ?><button class="btn btn-primary" name="action" value="class">Assign Class Teacher</button><?php } ?></form></div>
<?php if(staff_can('assignments.responsibility')||staff_can('responsibilities.manage')) { ?><div class="panel panel-body" id="responsibilities"><h3>Manage Responsibilities</h3><form method="post"><?php csrf_field(); ?><input type="hidden" name="year" value="<?= $year ?>"><input type="hidden" name="action" value="responsibility">
...
<label>Responsibility</label><select name="type" class="form-control academic-search" required><?php ar_options($dbh->query('SELECT id,Name Label FROM tblresponsibilitytypes WHERE Active=1'.(staff_can('responsibilities.manage')?'':' AND id IN (SELECT ResponsibilityTypeId FROM tblresponsibilityscope WHERE IsAcademic=1)').' ORDER BY Name')->fetchAll(PDO::FETCH_ASSOC)); ?></select>
...
<h4>Classify academic responsibilities</h4><p>Existing types stay nonacademic until explicitly classified. Only academic types are available to delegated Deans.</p>
<?php foreach($dbh->query('SELECT t.id,t.Name,COALESCE(s.IsAcademic,0) IsAcademic FROM tblresponsibilitytypes t LEFT JOIN tblresponsibilityscope s ON s.ResponsibilityTypeId=t.id ORDER BY t.Name') as $r) { ?><form method="post"><?php csrf_field(); ?><input type="hidden" name="year" value="<?= $year ?>"><input type="hidden" name="action" value="scope"><input type="hidden" name="type" value="<?= (int)$r['id'] ?>"><input type="hidden" name="academic" value="<?= $r['IsAcademic']?0:1 ?>"><?= academic_h($r['Name']) ?> <button class="btn btn-xs btn-default"><?= $r['IsAcademic']?'Mark nonacademic':'Mark academic' ?></button></form><?php } } ?></div><?php } ?>
